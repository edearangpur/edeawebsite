...
// Sidebar
	...
	// Events Slider Auto-rotation
if($('.events-slider-wrapper').length > 0) {
    let currentSlide = 0;
    ...
    for(let i = 0; i < totalSlides; i++) {
        dotsHtml += `<span class="slider-dot ${i === 0 ? 'active' : ''}" data-slide="${i}"></span>`;
    }
    dotsHtml += '</div>';
    $('.events-slider-wrapper').append(dotsHtml);
    
    // Auto rotation function
    function rotateSlide() {
        slides.eq(currentSlide).removeClass('active');
        $('.slider-dot').eq(currentSlide).removeClass('active');
    ...
    // Start auto-rotation (10 seconds)
    let slideInterval = setInterval(rotateSlide, 10000);
    ...
    $(document).on('click', '.slider-dot', function() {
        ...
        slideInterval = setInterval(rotateSlide, 10000);
    });
}
})(jQuery);